...
import { RefreshCw, ArrowDown, ArrowUp, Zap, Clock, CheckCircle, XCircle, ExternalLink, ArrowRight } from 'lucide-react'
...
const TransactionCard: React.FC<{ transaction: HistoryTransaction }> = ({ transaction }) => {
    ...
    const openInHashscan = () => {
        if (hasValidTxLink) {
            window.open(`https://hashscan.io/testnet/transaction/${transaction.txId}`, '_blank')
        }
    }
        ...
        <Card className={cn(
            ...
            <CardContent className={cn(
                ...
                <div className="space-y-4">
                            ...
                            <div className="flex items-center justify-between">
                                ...
                                <div className="flex flex-col items-center text-center">
                                    ...
                                    <div className="text-lg font-bold text-gray-900 dark:text-gray-100">
                                        {formatAmount(transaction.grossUSDC || transaction.netUSDC || 0)}
                                    </div>
                                </div>
                    ...
                    {(transaction.type === 'withdraw' || transaction.type === 'instant_withdraw') && (
                        <div className="bg-white/50 dark:bg-gray-800/50 rounded-lg p-4 border border-gray-200 dark:border-gray-700">
                            <div className="flex items-center justify-between">
                                {/* Step 1: hUSD Sent */}
                                <div className="flex flex-col items-center text-center">
                            ...
                            {transaction.type === 'instant_withdraw' && transaction.status === 'completed' && transaction.grossUSDC && (
                                <div className="mt-3 pt-3 border-t border-gray-200 dark:border-gray-700">
                                    <div className="flex justify-between text-sm">
                                        <span className="text-gray-600 dark:text-gray-400">Gross USDC:</span>
                                        <span className="font-medium">{formatAmount(transaction.grossUSDC)}</span>
                                    </div>
                                    <div className="flex justify-between text-sm text-red-600 dark:text-red-400">
                                        <span>Fee (1%):</span>
                                        <span>-{formatAmount(transaction.fee || 0)}</span>
                                    </div>
                                    <div className="flex justify-between text-sm font-semibold pt-1 border-t border-gray-200 dark:border-gray-700">
                                        <span>Net USDC:</span>
                                        <span className="text-green-600 dark:text-green-400">{formatAmount(transaction.netUSDC || 0)}</span>
                                    </div>
                                </div>
                            )}
                        </div>
...
export const HistoryCards: React.FC<HistoryCardsProps> = ({ userAccountId }) => {
    const {
        history,
        isLoading,
        error,
        ...
        <div className="space-y-6">
            ...
            <div className="space-y-4">
                {history.map((transaction, index) => (
                    <TransactionCard key={`${transaction.txId}-${index}`} transaction={transaction} />
                ))}
            ...
            {!isLoading && history.length === 0 && !error && (
                <div className="flex items-center justify-center py-12">
                    <div className="text-center">
                        ...
                        <h4 className="text-lg font-medium text-gray-900 dark:text-gray-100 mb-2">No Transaction History</h4>

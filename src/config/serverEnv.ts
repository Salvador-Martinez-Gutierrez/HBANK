...
const envSchema = z.object({
    ...
    // Network
    HEDERA_NETWORK: z.enum(['mainnet', 'testnet']).optional().default('testnet'),
    USE_REAL_TESTNET: z.union([z.literal('true'), z.literal('false')]).optional(),

    // Operator (legacy)
    OPERATOR_ID: z.string().regex(accountIdRegex, 'Invalid Hedera account ID').optional(),
    OPERATOR_KEY: z.string().min(10).optional(),

    // Wallets - Deposit
    DEPOSIT_WALLET_ID: z.string().regex(accountIdRegex, 'Invalid Hedera account ID'),
    DEPOSIT_WALLET_KEY: z.string().min(10),

    // Wallets - Withdrawals
    INSTANT_WITHDRAW_WALLET_ID: z.string().regex(accountIdRegex, 'Invalid Hedera account ID'),
    INSTANT_WITHDRAW_WALLET_KEY: z.string().min(10),
    STANDARD_WITHDRAW_WALLET_ID: z.string().regex(accountIdRegex, 'Invalid Hedera account ID').optional(),
    STANDARD_WITHDRAW_WALLET_KEY: z.string().min(10).optional(),

    // Wallets - Treasury & Emissions
    TREASURY_ID: z.string().regex(accountIdRegex, 'Invalid Hedera account ID').optional(),
    ...
    RATE_PUBLISHER_ID: z.string().regex(accountIdRegex, 'Invalid Hedera account ID').optional(),
    ...
    // Topics
    TOPIC_ID: z.string().regex(topicIdRegex, 'Invalid Hedera topic ID').optional(),
    WITHDRAW_TOPIC_ID: z.string().regex(topicIdRegex, 'Invalid Hedera topic ID').optional(),
...
if (!envResult.success) {
    logger.error('❌ Invalid server environment configuration', envResult.error.flatten())
    throw new Error('Invalid server environment configuration. Check logs above.')
}
...
export const serverEnv = {
    ...
    // Operators/Wallets
    operators: {
        legacy: env.OPERATOR_ID && env.OPERATOR_KEY ? {
            accountId: env.OPERATOR_ID,
            privateKey: env.OPERATOR_KEY,
        } as OperatorConfig : undefined,
        deposit: {
        ...
        standardWithdraw: env.STANDARD_WITHDRAW_WALLET_ID && env.STANDARD_WITHDRAW_WALLET_KEY ? {
            accountId: env.STANDARD_WITHDRAW_WALLET_ID,
            privateKey: env.STANDARD_WITHDRAW_WALLET_KEY,
        } as OperatorConfig : undefined,
        treasury: env.TREASURY_ID && env.TREASURY_KEY ? {
        ...
        ratePublisher: env.RATE_PUBLISHER_ID && env.RATE_PUBLISHER_KEY ? {
...
if (serverEnv.isProduction) {
    if (serverEnv.cache.type === 'memory') {
        console.warn('[serverEnv] ⚠️ Using in-memory cache in production. Use Redis for multi-instance deployments.')
    }
    ...
    if (!serverEnv.sentry.enabled) {
        console.warn('[serverEnv] ⚠️ Sentry monitoring is disabled in production')
    }
}
...
export const ensureTestnetAccess = () => {
    if (!serverEnv.useRealTestnet) {
        throw new Error('Hedera access is disabled because USE_REAL_TESTNET=false')
    }
}
...
console.log('[serverEnv] ✅ Environment configuration loaded successfully')
console.log(`[serverEnv]    Environment: ${serverEnv.nodeEnv}`)
console.log(`[serverEnv]    Hedera Network: ${serverEnv.hedera.network}`)
console.log(`[serverEnv]    Cache: ${serverEnv.cache.type}`)
console.log(`[serverEnv]    Telegram: ${serverEnv.telegram.enabled ? 'enabled' : 'disabled'}`)
console.log(`[serverEnv]    Sentry: ${serverEnv.sentry.enabled ? 'enabled' : 'disabled'}`)

...
export async function POST(req: NextRequest): Promise<NextResponse> {
    try {
        ...
        logger.info('🔍 Verifying HUSD transfer to emissions wallet...')

        const emissionsWalletId = ACCOUNTS.emissions
        logger.info('📋 Using emissions wallet:', emissionsWalletId)

        // Check for transfers in the last 10 minutes
        const since = new Date(Date.now() - 10 * 60 * 1000).toISOString()

        logger.info('🔍 [STANDARD WITHDRAW] HUSD transfer search parameters:', {
            from: userAccountId,
        ...
        logger.info('✅ HUSD transfer verified for standard withdrawal')
        ...
        try {
            ...
            logger.error('Failed to publish withdrawal request:', publishError)
        ...
        logger.info('✅ Withdrawal request processed successfully:', {
            requestId,
            withdrawRequestTxId,
            unlockAt,
        })

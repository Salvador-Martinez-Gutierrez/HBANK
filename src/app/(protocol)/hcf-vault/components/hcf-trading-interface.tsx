...
export function HCFTradingInterface() {
    ...
    const fromToken = activeTab === 'mint' ? 'USDC' : 'fUSD'
    const toToken = activeTab === 'mint' ? 'fUSD' : 'USDC'
    const fromIcon = activeTab === 'mint' ? '/usdc.svg' : '/usdc.svg|purple'
    const toIcon = activeTab === 'mint' ? '/usdc.svg|purple' : '/usdc.svg'
    ...
    const handleFromAmountChange = (value: string) => {
        ...
        if (value && !isNaN(parseFloat(value))) {
            ...
            // Always 1:1 rate for HCF Vault
            calculatedTo = parseFloat(value)
            ...
            if (
                ...
                // At 1:1 rate, max hCF = max USDC
                setFromAmount(maxInstantWithdrawable.toFixed(6))
                calculatedTo = maxInstantWithdrawable
            }
    ...
    // Calculate USD value - simplified for 1:1 rate
    const getUsdValue = (amount: string) => {
        ...
        // At 1:1 rate, everything is equal in USDC value
        const formatted = formatCurrency(numericAmount, 6)
        return `$${formatted}`
    }
    ...
    const renderTabContent = () => (
        <div className='space-y-4'>
            ...
            {activeTab === 'redeem' && redeemType === 'instant' && (
                <div
                ...
                >
                    <div className='text-sm text-blue-800'>
                        ...
                        {formatCurrency(maxInstantWithdrawable, 6)} fUSD at 1:1
                        rate)
                    </div>
                    {isLoadingMaxAmount && (

  ...
  useEffect(() => {
    const fetchBalances = async () => {
      ...
      if (!isConnected || !accountId) {
        console.log('⚠️ [useTokenBalances] Not connected or no accountId, resetting balances')
        setBalances({
          USDC: '0',
          hUSD: '0'
        })
      ...
      try {
        ...
        console.log('📡 [useTokenBalances] Response status:', response.status)
        ...
        if (response.ok) {
          ...
          if (data.tokens && Array.isArray(data.tokens)) {
            ...
            for (const token of data.tokens) {
              ...
              if (token.token_id === TOKEN_IDS.USDC) {
                ...
                console.log(`✅ [useTokenBalances] Found USDC: ${balance} (balance: ${token.balance}, decimals: ${decimals})`)
              ...
              // Check if this is hUSD
                ...
                const divisor = Math.pow(10, decimals)
                const balance = (token.balance / divisor).toFixed(2)
                console.log(`✅ [useTokenBalances] Found hUSD: ${balance} (balance: ${token.balance}, decimals: ${decimals})`)
          ...
          console.warn(`❌ [useTokenBalances] API request failed with status: ${response.status}`)

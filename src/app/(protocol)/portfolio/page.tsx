  ...
  useEffect(() => {
    const run = async () => {
      if (!isConnected || !accountId) {
        setBalances({ hbar: '0', usdc: '0', husd: '0' })
        return
      }
      setLoading(true)
  ...
  const tokenConfigs = [
    {
      ...
      icon: '/hbar.webp',
      grayscale: false,
      getValue: () => formatUsd((parseFloat(balances.hbar || '0') || 0) * (hbarPrice || 0)),
    },
    {
      key: 'usdc' as const,
      name: 'USDC',
    ...
    },
    {
      ...
      icon: '/usdc.svg',
      grayscale: true,
      getValue: () => formatUsd(parseFloat(balances.husd || '0') || 0),
    },
  ]
            ...
            <CardContent>
              <div className="flex items-end justify-between">
                <div>
                  ...
                  <div className="text-2xl font-semibold">{loading ? '—' : balances[t.key]}</div>
                </div>
                <div className="text-right">
                  <div className="text-sm text-muted-foreground">Value</div>
                  <div className="text-xl font-medium">{loading ? '—' : t.getValue()}</div>
                </div>
              </div>
            </CardContent>
          </Card>
